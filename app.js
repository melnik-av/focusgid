...
async function loadWalkData(walkId, role) {
    ...
    if (walk.audio_track_id) {
        ...
        if (e1) {
            console.error('❌ Ошибка загрузки трека 1:', e1);
        } else {
            track1 = t1;
            console.log('🎵 Трек 1:', track1);
        }
    }
    ...
    if (!track) {
        ...
        console.error('walk.audio_track_id:', walk.audio_track_id);
        console.error('walk.audio_track_id_2:', walk.audio_track_id_2);
        throw new Error('Аудиотрек не найден. Назначьте трек в админке.');
    }
    
    if (!track.file_url) {
        console.error('❌ У трека нет file_url!');
        throw new Error('У трека нет файла');
    }
...
async function loadWalk(walkId, role) {
    try {
        ...
        showError(error.message || 'Трек не найден');
    }
}
...
async function activateByKey(key) {
    try {
        ...
        if (error || !purchase) {
            showKeyForm('❌ Неверный ключ');
            return;
        }
        
        if (purchase.expires_at && new Date(purchase.expires_at) < new Date()) {
            showKeyForm('⏰ Срок истек');
            return;
        }
...
async function init() {
    ...
    if (accessKey) {
        ...
        showKeyForm('Введите ключ доступа');
    }
}
